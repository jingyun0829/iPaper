...
def fetch_abstract_crossref(doi: str) -> str:
    ...
    try:
        r = requests.get(
            ...
            headers={"User-Agent": "PaperPulse/1.0 (mailto:research@example.com)"},
            timeout=8,
        )
        if r.status_code == 200:
            ...
            if abstract:
                # Remove JATS XML tags like <jats:p>
...
def fetch_feed(feed_config: dict) -> list[dict]:
    ...
    try:
        ...
        for entry in feed.entries[:15]:  # max 15 per feed per run
            ...
            # If abstract is too short (just page info), fetch from CrossRef
            if len(abstract) < 100 and doi:
                abstract = fetch_abstract_crossref(doi)
                if abstract:
                    print(f"      📖 Fetched abstract via CrossRef")
